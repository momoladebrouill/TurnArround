...
class lutin:  # Les météorites
    def __init__(self):
        ...
        self.pos=len(lutins)
        if lutins:
            d=distance(self,lutins[0])
            self.angl=math.acos((self.x-lutins[0].x)/d.dist())
            if lutins[0].y > self.y :
    ...
    def find_angl(self):
        if lutins:
            dista=math.sqrt((self.x-lutins[0].x)**2+(self.y-lutins[0].y)**2)
            if dista!=0:
                d=distance(self,lutins[0])
    ...
    def fd(self):  # Suivre son instinct
        ...
        if self==lutins[0]:
            ...
            self.angl+=math.pi/dista

            self.x=math.cos(self.angl)*dista+lutins[0].x
            self.y=math.sin(self.angl)*dista+lutins[0].y
